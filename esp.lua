...
local function SaveWorkspaceSetting(key, value)
    ...
    if not success then
        warn("Save failed: " .. key .. " " .. tostring(err))
    end
end
...
local function MakeToggle(name, flag, default)
    MainSection:CreateToggle({
        ...
        Callback = function(v)
            Settings[flag] = v
            SaveWorkspaceSetting(flag, v)
            if flag == "Enabled" and not v then
...
MakeToggle("Enable ESP", "Enabled", Settings.Enabled)
MakeToggle("Box ESP", "BoxESP", Settings.BoxESP)
MakeToggle("Skeleton ESP", "SkeletonESP", Settings.SkeletonESP)
MakeToggle("Name ESP", "NameESP", Settings.NameESP)
MakeToggle("Health Bar", "HealthESP", Settings.HealthESP)
MakeToggle("Tracer ESP", "TracerESP", Settings.TracerESP)
MakeToggle("Head Point", "PointESP", Settings.PointESP)
MakeToggle("Team Check", "TeamCheck", Settings.TeamCheck)
MakeToggle("Show Distance", "ShowDistance", Settings.ShowDistance)
...
local colorItems = {
    {"Seeker / Killer", "Seeker", Colors.Seeker},
    {"Hider", "Hider", Colors.Hider},
    {"Innocent", "Innocent", Colors.Innocent},
    {"Traitor", "Traitor", Colors.Traitor},
    {"Police / SWAT / Sheriff", "Police", Colors.Police},
    {"Juggernaut", "Juggernaut", Colors.Juggernaut},
    {"No Role (Spawn)", "NoRole", Colors.NoRole},
    {"Unknown (fallback)", "Unknown", Colors.Unknown},
}
for _, item in ipairs(colorItems) do
    ColorsSection:CreateColorPicker({
        ...
        Callback = function(v)
            Colors[item[2]] = v
            SaveWorkspaceSetting(item[2] .. "Color", v)
            if item[2] == "Seeker" then
                Colors.Killer = v
                SaveWorkspaceSetting("Killer", v)
            elseif item[2] == "Police" then
                Colors.Swat = v
                Colors.Sheriff = v
