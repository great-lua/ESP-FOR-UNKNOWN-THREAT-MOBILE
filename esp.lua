-- ESP FOR UNKNOWN THREAT MOBILE (Rayfield version)
-- by Great | discord.gg/qR7ABr7f

local Rayfield = loadstring(game:HttpGet("https://raw.githubusercontent.com/xHQLz/Rayfield/main/source.lua"))()
...
-- ============================================================
-- СОХРАНЕНИЕ НАСТРОЕК В WORKSPACE
-- ============================================================
...
local function GetPlayerRole(player)
    ...
    if not roleStr then return "NoRole" end
...
local function GetTracerOrigin()
    ...
    if UserInputService.TouchEnabled and origin == "Mouse" then
        origin = "Center"
    end
...
local function UpdateESP(player)
    ...
    local topPos, topOnScreen, topZ = Camera:WorldToViewportPoint((cf * CFrame.new(0, size.Y/2, 0)).Position)
    local bottomPos, bottomOnScreen, bottomZ = Camera:WorldToViewportPoint((cf * CFrame.new(0, -size.Y/2, 0)).Position)

    if not topOnScreen or not bottomOnScreen or topZ < 0 or bottomZ < 0 then
        for _, line in pairs(esp.Box) do line.Visible = false end
        return
    end

    local screenSize = bottomPos.Y - topPos.Y
    ...
    if Settings.NameESP then
        ...
        esp.Name.Position = Vector2.new(topPos.X, topPos.Y - 20)
    ...
    if Settings.SkeletonESP then
        ...
        local function drawLine(fromPart, toPart, line)
            ...
            local fromPos, fromOnScreen, fromZ = Camera:WorldToViewportPoint(fromPart.Position)
            local toPos, toOnScreen, toZ = Camera:WorldToViewportPoint(toPart.Position)
            if not fromOnScreen or not toOnScreen or fromZ < 0 or toZ < 0 then
                line.Visible = false
                return
            end
...
local Window = Rayfield:CreateWindow({
    ...
    Icon = 0, -- иконка (можно поставить любой номер из библиотеки)
    LoadingTitle = "Loading ESP...",
    LoadingSubtitle = "by Great",
    ConfigurationSaving = {
        Enabled = false, -- мы сохраняем в workspace
    },
    KeySystem = false,
    KeySettings = {
        OpenKey = Enum.KeyCode.LeftControl,
        -- на мобиле можно открыть по кнопке
    },
})

-- Адаптивный размер для мобильных (автоматически подстраивается)
local Tabs = {
    Main = Window:CreateTab("Main", 0),
    Visuals = Window:CreateTab("Visuals", 0),
    Settings = Window:CreateTab("Settings", 0),
    Config = Window:CreateTab("Config", 0),
}

-- ============================================================
-- ВКЛАДКА MAIN
-- ============================================================
local mainSection = Tabs.Main:CreateSection("ESP Toggles")
...
-- ВКЛАДКА VISUALS
-- ============================================================
local visualSection = Tabs.Visuals:CreateSection("Box Settings")
...
local tracerSection = Tabs.Visuals:CreateSection("Tracer Settings")
...
local healthSection = Tabs.Visuals:CreateSection("Health Settings")
...
local pointSection = Tabs.Visuals:CreateSection("Point Settings")
...
local skeletonSection = Tabs.Visuals:CreateSection("Skeleton Settings")
...
-- ВКЛАДКА SETTINGS
-- ============================================================
local settingsSection = Tabs.Settings:CreateSection("General")
...
local colorsSection = Tabs.Settings:CreateSection("Role Colors")
...
-- ============================================================
-- ВКЛАДКА CONFIG (управление)
-- ============================================================
local configSection = Tabs.Config:CreateSection("Actions")
